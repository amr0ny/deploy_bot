...
from src.provider.manager import AsyncProviderManager, TaskManager
...
async def main():
    ...
    provider = AsyncSnaptikProvider(timeouts=timeout_config)
    ...
    task_queue = QueueFactory.create(
        QueueType.IN_MEMORY, name=config.queue_name, maxsize=config.queue_maxsize
    )
    ...
    db_middleware = DbMiddleware(session_maker)
    ...
    task_browser_factory_middleware = DependencyMiddleware(
        "task_browser_factory", task_browser_factory
    )
    ...
    await setup_scheduler(bot, session_maker, task_queue, manager, task_browser_factory, config.channel_id)
